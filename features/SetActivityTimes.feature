Feature: Set activity times
  ...
  Scenario: set start week successfully
    Given employee "barc" is logged in
    And there exists an activity with id "1"
    When the start week is set to Week 1 on activity with id "1"
    Then the activity with id "1" has start week Week 1

  Scenario: start week set after end week.
    Given employee "barc" is logged in
    And there exists an activity with id "1"
    And the activity id "1" has end week Week 5
    When the start week is set to Week 6 on activity with id "1"
    Then the error message "start week cannot be after end week" is given

  Scenario: set end week succesfully
    Given employee "barc" is logged in
    And there exists an activity with id "1"
    When the end week is set to Week 3 on activity with id "1"
    Then the activity with id "1" has end week Week 3

  Scenario: end week set before start week.
    Given employee "barc" is logged in
    And there exists an activity with id "1"
    And the activity has start week Week 5
    When the end week is set to Week 3 on activity with id "1"
